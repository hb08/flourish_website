...
class PlotController extends BaseController
{
    ...
    public function startPlotter()
    {
        if (\Session::get('ustatus') == 1) {
            ...
            return \View::make('pages.plot_begin', array(
                'title' => 'Garden Plotter | Flourish – Your Florida Gardening Guide',
                'count' => $count,
                'userList' => $userList
            ));
        } else {
    ...
    public function newPlot()
    {
        if (\Session::get('ustatus') == 1) { // User Logged In
            ...
            sort($newList);
            return \View::make('pages.plot_new', array(
                'title' => 'New Garden | Flourish – Your Florida Gardening Guide',
                'input' => $input,
                'plants' => $newList
            ));
    ...
    public function saveGarden()
    {
        ...
        $check = \DB::table('garden_plots')->where('user_id', \Session::get('user'))->where('garden_name',
            $name)->pluck('garden_id');
        $i = 1;
    ...
    public function editPlot($garden)
    {
        ...
        $thisGarden = \DB::table('garden_plots')->where('garden_id', $gardenId)->select('garden_name as name',
            'garden_code as code', 'garden_size as size')->get();
        // Array waiting for Garden Numbers
        $gardenNumber = array();
        $plants = array();
        $uid = \Session::get('user');
        ...
        // Alphabetize the list
        sort($newList);
        return \View::make('pages.plot.plot_edit', array(
            'title' => 'Edit Garden | Flourish – Your Florida Gardening Guide',
            'size' => $size,
            'code' => $code,
            'plants' => $newList,
            'gid' => $gardenId,
