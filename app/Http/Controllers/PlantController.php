...
class PlantController extends BaseController
{
    ...
    public function searchPlants()
    {
        ...
        return \View::make('pages.search', array(
            'title' => 'Plant Directory | Flourish – Your Florida Gardening Guide',
            'plants' => $plantList,
            'count' => $count,
            'filter' => $filter,
            'zip' => $zip,
    ...
    public function plantDetails($plantId)
    {
        ...
        return \View::make('pages.details', array(
            'title' => ($plantChart["0"]->plant_name . ' | Flourish – Your Florida Gardening Guide'),
            'chart' => $plantChart["0"],
            'thisPanel' => 'details',
            'imgSrc' => $altImageSrc["0"],
            'info' => $plantInfo["0"],
            'img' => $image_location,
            'diff' => $plant_diff
        ));
    ...
    public function removePlant()
    {
        ...
        $lid = \Input::get('list');
        if ($lid == 'gardens') {
            $lnum = \DB::table('garden_plots')->where('user_id', \Session::get('user'))->where('garden_name',
                $lname)->pluck('garden_id');
