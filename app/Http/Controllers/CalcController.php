...
class CalcController extends BaseController
{
    ...
    public function showMilestone()
    {
        if (\Session::get('ustatus') == 1) {
            ...
            // Plants
            $listId = array(
                'gid' => \DB::table('user_lists')->where('user_id', $uid)->where('user_listname',
                    'Growing')->pluck('list_id'),
                'wid' => \DB::table('user_lists')->where('user_id', $uid)->where('user_listname',
                    'Waiting')->pluck('list_id'),
                'mlid' => \DB::table('user_lists')->where('user_id', $uid)->where('user_listname',
            ...
            // Ged all Milestones
            $dbMile = \DB::table('user_milestones')->where('user_id', \Session::get('user'))->select('plant_id', 'm_type',
                'end_date')->get();
            // Containers
            $holdP = array();
            $holdMsprout = array();
            ...
            // Plant name list
            return \View::make('pages.calendar', array(
                'title' => 'My Garden Calendar | Flourish – Your Florida Gardening Guide',
                'pnames' => $plantNames,
        ...
        for ($i = 0; $i < count($milestones); $i++) {
            if ($milestones[$i] == 'Planting') {
                ...
                $days = array(
                    array(
                        'type' => $types['fert'],
                        'time' => \DB::table('plant_list')->where('plant_id', $id)->pluck('fertilize_time')
                    ),
                    array(
                        'type' => $types['thin'],
                        'time' => \DB::table('plant_list')->where('plant_id', $id)->pluck('thin_time')
                    ),
                    array(
                        'type' => $types['sprout'],
                        'time' => \DB::table('plant_list')->where('plant_id', $id)->pluck('sprout_time')
                    )
                );
                foreach ($days as $day) {
                    if (!empty($day['time'])) {
